...
5. Expose an API for point-to-point high performance encrypted communication using IPv4 for  
transport.
...
the device to encrypt data and send messages to peers. For V1, we will initially design for the C 
API. The Rust client library is a bonus that we gain from the Rust -> C compilation. The C API 
...
Initially, Unix Domain Sockets and shm will be used for IPC between the daemon and client library.
