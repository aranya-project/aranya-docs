...
The set can grow or shrink freely. The daemon SHOULD validate that a new finalizer set does not lose BFT safety (e.g., shrinking below 4, the smallest set size with f=1) before calling `UpdateFinalizerSet`. **[FSET-008]** This validation is the caller's responsibility, not the policy's — the policy does not enforce consensus-specific thresholds.
...
All finalization triggers use the same scheduling mechanism: an effect that sets a delay (a fixed time duration, e.g., "wait 30s") before the next finalization round. The daemon tracks one pending schedule at a time -- a shorter delay overwrites the current schedule. Finalization scheduling MUST use a delay (not wall clock time) because there is no guarantee all daemons agree on the current wall clock time; the daemon MUST track the delay locally. **[TRIG-003]** Delay-based scheduling does not require a real-time clock or wall clock — it only requires a monotonic counter (e.g., CPU cycle counter) to measure elapsed time. The schedule is not stored in the FactDB because it is daemon-local — any daemon that processes the delay effect will independently wait the specified duration before initiating the next finalization round. If multiple daemons initiate finalization concurrently, the consensus protocol handles this via its proposer selection mechanism.

Finalizers with different clock rates may disagree on exactly when the delay expires. This does not affect safety — a single finalizer cannot prevent a finalization round. As long as a quorum of finalizers agree that sufficient time has passed, finalization will proceed. Eventually, even the finalizer with the slowest clock will reach the delay threshold, at which point all finalizers agree finalization should occur. By not relying on wall clocks anywhere in the consensus design, the protocol is immune to NTP spoofing attacks — an attacker who manipulates a finalizer's wall clock time cannot affect finalization scheduling or consensus behavior.

An alternative approach is command-count-based scheduling — triggering finalization after a certain number of new commands rather than after a time delay. This avoids unnecessary finalization rounds when no new commands exist. However, command-count scheduling requires finalizers to be up-to-date with the graph to know a new round is needed, while delay-based scheduling triggers periodically regardless of graph state, which is more robust when finalizers may have stale views of the graph. A hybrid approach (finalize after N commands OR after delay D, whichever comes first) could combine the advantages of both.
...
2. **Daemon restart.** When a finalizer daemon starts or restarts, it MUST attempt to trigger finalization unconditionally. **[TRIG-004]** The daemon MUST NOT skip the attempt based on local delay state. This is necessary because the finalization schedule effect is ephemeral — if all finalizers go offline simultaneously, all lose their pending schedules. Unconditionally initiating on restart prevents finalization from halting permanently in this case. Since daemons are long-lived processes, restarts should be infrequent, so the cost of an occasional unnecessary finalization round is acceptable. If there are no unfinalized commands since the last Finalize, the proposer's proposal will be for the same parent and the round will succeed harmlessly or other finalizers will already be at a higher height and Malachite will handle the mismatch. **Open question:** other finalizers could reject premature finalization attempts by prevoting nil (e.g., if the attempt is well before the next scheduled finalization), but the exact rejection criteria have not been defined.
...
The initiator is unlikely to be the proposer (e.g., 1 in 7 chance) — the proposer MUST be selected by the deterministic proposer rotation of the BFT algorithm, independent of which finalizer initiated the round (see [Agreement](#phase-1-agreement)). **[TRIG-007]** If multiple finalizers initiate concurrently, the consensus protocol MUST resolve to a single proposal. **[TRIG-005]** Redundant initiation attempts are harmless.
...
| **Slow clock attack** | A malicious finalizer deliberately runs a slow clock to delay initiating finalization rounds when it is the initiator, hoping to slow down the finalization cadence | A single finalizer cannot delay finalization. Delay-based scheduling is daemon-local — each finalizer independently tracks its own delay. As long as a quorum of finalizers believe the delay has elapsed, finalization proceeds regardless of the slow finalizer. The slow finalizer only affects rounds where it is the designated proposer (1/n chance), and even then the round times out and rotation selects the next proposer. Up to `f` finalizers with arbitrarily slow clocks are tolerated. |
| **NTP spoofing** | An attacker manipulates a finalizer's wall clock time (e.g., via NTP spoofing) to disrupt finalization scheduling or consensus behavior | The consensus protocol does not rely on wall clocks at any point — scheduling uses monotonic counters (CPU cycle counters) for delay measurement, and consensus messages use logical sequence numbers (height, round), not timestamps. Manipulating a finalizer's wall clock has no effect on finalization scheduling or consensus. |
