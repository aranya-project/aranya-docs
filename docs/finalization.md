...
- **Unsigned Finalize command** -- The Finalize command envelope without signatures, constructed by the proposer (see [Certified Command Implementation](#certified-command-implementation)). By including the command in the proposal, all finalizers sign the exact same content. Signatures are attached during the precommit phase.
...
6. **Commit.** As precommit messages arrive, each finalizer accumulates signatures. Once a finalizer has collected at least 3 signatures (quorum), it builds the certified envelope and commits at the agreed-upon parent using action-at-parent. Different finalizers may commit with different signature subsets but all produce the same command ID (see [Certified Commands](#certified-commands)). Duplicates are dropped during sync.
