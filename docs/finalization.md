...
Combining signatures with precommits eliminates a separate signature collection phase. The trade-off is that signature computation is wasted when consensus fails (the finalizer signed the payload but the round didn't reach agreement). This is acceptable because consensus succeeds in the common case and the wasted work is only a single signature per failed round.
