...
4. Provide stable APIs that allow devices to interact with Aranya.
...
The client library exists inside the device's process and is used to interact with the
daemon. The client library also includes a light wrapper around AFC allowing the device
...
<img src="{{ '/assets/images/new_daemon_arch_block.png' | relative_url }}" class="doc-image" alt="daemon subsystems" />

<img src="{{ '/assets/images/new_daemon_arch_block_detail.png' | relative_url }}" class="doc-image" alt="daemon subsystems with additional detail" />
...
This approach allows the device API to drive the configuration of the daemon, and
...
device by providing an invite code instead of passing a KeyBundle.
...
- CreateChannel(team_id, net_identifier, label) -> channel_id - Open a channel to the destination with the given label. The device API transparently handles sending the ephemeral command to the
peer. Channel keys are automatically rotated after a specific byte count.
- DeleteChannel(team_id, channel_id) -> bool - Close a channel with the given ID. The device API transparently handles sending the ephemeral command to the peer.
...
There will be 4 different device roles with the following set of permissions for each.

`owner`
- root device (all permissions excluding sending data on AFC label)
- create/close team
- add/remove devices
...
- assign device role to devices in team
- assign/revoke AFC labels
- assign/remove addresses/names for AFC
- cannot send data on AFC labels
- sync
...
- `device` - a computer or person operating the instance.
- `IPC` - inter-process communication.
- `policy` - an Aranya policy, containing the logic and rules of the system.
- `sync` - a request to synchronize the commands on the control plane. Syncs are currently pull only, so the device that requests a sync receives commands from the requester.
