...
- **Vendored C/C++** - Upstream library source included in -sys crates. There is no established standard for auditing vendored C/C++ code within Rust crates — cargo-vet treats vendored C files as opaque content and its audit criteria are defined in terms of Rust behavior. The following approaches are listed in order of practicality:
  1. **Verify against upstream source** — Compare vendored code against the upstream git tag for the specified version. Any delta may indicate tampering. Note: a coordinated attacker controlling both the crate and upstream could bypass this.
  2. **Check upstream project security posture** — Review the upstream project's [OpenSSF Scorecard](https://scorecard.dev/), whether it uses static analysis, has branch protections, and has a vulnerability disclosure process.
  3. **Check compiler hardening flags** — Verify the -sys crate's build scripts use appropriate flags per the [OpenSSF Compiler Hardening Guide](https://best.openssf.org/Compiler-Hardening-Guides/Compiler-Options-Hardening-Guide-for-C-and-C++.html) (e.g., `-fstack-protector-strong`, `-D_FORTIFY_SOURCE`, `-fPIE`).
  4. **Run static analysis tools** — Tools like `clang-tidy` (with `bugprone-*`, `cert-*` checkers), `cppcheck`, or [CodeQL](https://codeql.github.com/) can find common C/C++ vulnerabilities (buffer overflows, use-after-free, integer overflows) in vendored source.
