...
**NOTE:** actions and effects are the only public parts of a policy. Everything else is an implementaion detail that can be changed without updating the application. However, any structs or enums referenced in actions or effects should be considered public - changes to them would require updating the application.
