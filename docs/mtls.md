...
|---|---|---|---|---|---|---|---|
| **Outbound — server cert** | **Custom.** Set to team ID (base58), not a hostname. **[MTLS-084]** | Standard mTLS. Client validates server cert against the team's trust anchors. The client already knows which team it is connecting for. **[MTLS-061]** | Standard TLS. If cert has SANs, at least one (IP or DNS) must match. No SANs on cert: skip SAN checks. | IP SANs checked against peer's resolved IP. **[MTLS-090]** | DNS SANs checked against peer's hostname (if known). **[MTLS-090]** | Connection established. | Handshake fails. |
| **Inbound — client cert (handshake)** | Set by the connecting client. Used to select per-team trust anchors. **[MTLS-055]** | **Custom.** Per-team trust anchors selected by SNI. Standard mTLS uses a single global trust store. **[MTLS-087]** | No SAN checks. Standard mTLS also does not check client SANs. | Not checked. | Not checked. | Connection established. | Handshake fails. |
| **Inbound — client cert (reverse reuse)** | N/A — already established. | Already validated during handshake. | **Custom.** Not part of standard TLS. If cert has SANs, at least one (IP or DNS) must match. No SANs on cert: skip SAN checks. | IP SANs checked against peer's connecting IP. **[MTLS-073, MTLS-075]** | DNS SANs resolved and checked against peer's connecting IP. **[MTLS-074]** | Connection reused in reverse. | Connection NOT reused. New outbound connection attempted instead. **[MTLS-066]** Inbound connection remains open. **[MTLS-077]** |
...
| **Long-lived connections per team** | One connection between two peers, multiplexing via QUIC streams. Standard TLS (especially HTTPS) typically establishes a new connection per request, resulting in frequent cert validation. | At most one inbound and one outbound long-lived connection per (peer, team) pair. **[MTLS-060]** | Each team requires different certs and trust chains. Prevents cross-team cert mismatch, limits blast radius of CA compromise. Long-lived connections avoid the bandwidth overhead of a TLS handshake per sync request. | Mixed. Positive: compromised CA for one team cannot affect other teams' connections. **Deliberate regression:** long-lived connections mean certs are validated less frequently than standard TLS. An expired or compromised cert may not be caught until the connection is re-established. Periodic connection re-establishment is planned to mitigate this (see [Future Work](#future-work)). |
