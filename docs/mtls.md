...
| **Compromised CA** | Attacker compromises a CA in the cert chain and issues fraudulent device certs. | Partially mitigated. Per-team connections limit blast radius to teams using the compromised cert chain. **[MTLS-060]** | Cert revocation to invalidate certs issued by the compromised CA. |
