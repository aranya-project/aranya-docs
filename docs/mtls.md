...
|---|---|---|---|---|---|---|
| **Outbound — server cert** | **Custom.** Set to team ID (base58), not a hostname. **[MTLS-084]** | **Custom.** Per-team trust anchors selected by team ID. Standard TLS uses a single global trust store. **[MTLS-061]** | Standard TLS. At least one SAN (IP or DNS) must match. IP SANs checked against peer's resolved IP. **[MTLS-090]** | Standard TLS. At least one SAN (IP or DNS) must match. DNS SANs checked against peer's hostname (if known). **[MTLS-090]** | Connection established. | Handshake fails. |
| **Inbound — client cert (handshake)** | Set by the connecting client. Used to select per-team trust anchors. **[MTLS-055]** | **Custom.** Per-team trust anchors selected by SNI. Standard mTLS uses a single global trust store. **[MTLS-087]** | Not checked. Standard mTLS also does not check client SANs. | Not checked. | Connection established. | Handshake fails. |
| **Inbound — client cert (reverse reuse)** | N/A — already established. | Already validated during handshake. | **Custom.** Not part of standard TLS. At least one SAN (IP or DNS) must match. IP SANs checked against peer's connecting IP. **[MTLS-073, MTLS-075]** | **Custom.** Not part of standard TLS. At least one SAN (IP or DNS) must match. DNS SANs resolved and checked against peer's connecting IP. **[MTLS-074]** | Connection reused in reverse. | Connection NOT reused. New outbound connection attempted instead. **[MTLS-066]** Inbound connection remains open. **[MTLS-077]** |
