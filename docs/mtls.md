...
Call `set_cert` again with new file paths per **[MTLS-029]**. The daemon overwrites cert files, replaces the keystore key, and updates the resolver and verifier caches. Existing connections for the team are removed from the connection map so new sync requests will create fresh connections using the new cert. **[MTLS-085]** In-progress sync requests/responses on old connections are allowed to complete — quinn's `SendStream` and `RecvStream` hold internal references to the connection, keeping it alive until all active streams close. No explicit drain tracking is needed; the old connection is automatically cleaned up when the last in-progress sync completes. The server's `ResolvesServerCert` and `ClientCertVerifier` caches are updated immediately, so new inbound connections also use the new cert.
...
|---|---|---|---|---|
| **SNI usage** | SNI contains the server's hostname for virtual hosting and SAN verification. | SNI contains the team ID (base58) for team-based cert selection. Not a valid hostname. | Each team has its own certs and trust chain. SNI is the only signal available during the TLS handshake to identify which team's certs to use. | Neutral. SNI is cleartext in TLS 1.3 regardless, and team ID is not sensitive. Team existence oracle mitigated by **[MTLS-057]**. |
