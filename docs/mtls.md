...
|---|---|---|---|---|---|
| **Outbound — server cert** | Standard mTLS. Server cert validated against team's trust anchors (team selected by SNI). **[MTLS-061]** | Standard TLS, **custom input.** If cert has IP SANs: peer's resolved IP MUST match one. Standard TLS checks against SNI; we check against peer's actual address because SNI contains the team ID. **[MTLS-090]** | Standard TLS, **custom input.** If cert has DNS SANs and peer was configured by hostname: hostname MUST match one. **[MTLS-090]** | Connection established. | Handshake fails. |
| **Inbound — client cert (handshake)** | **Custom.** Client cert validated against per-team trust anchors (team selected by SNI from ClientHello). Standard mTLS uses a single global trust store. **[MTLS-087]** | Not checked during handshake. Standard mTLS also does not check client SANs. | Not checked during handshake. | Connection established. | Handshake fails. |
| **Inbound — client cert (reverse reuse)** | Already validated during handshake. | **Custom.** Not part of standard TLS. If cert has IP SANs: peer's connecting IP MUST match one. **[MTLS-073]** IPv4-mapped IPv6 compared against IPv4 equivalent. **[MTLS-075]** | **Custom.** Not part of standard TLS. If cert has DNS SANs: resolved IP MUST match peer's connecting IP. **[MTLS-074]** | Connection reused in reverse. | Connection NOT reused. New outbound connection attempted instead. **[MTLS-066]** Inbound connection remains open. **[MTLS-077]** |
