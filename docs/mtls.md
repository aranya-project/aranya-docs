...
|---|---|---|---|---|---|---|---|
| **Outbound — server cert** | **Custom.** Set to team ID (base58), not a hostname. **[MTLS-084]** | **Custom.** Per-team trust anchors selected by team ID. Standard TLS uses a single global trust store. **[MTLS-061]** | Standard TLS. If cert has SANs, at least one (IP or DNS) must match. No SANs on cert: skip SAN checks. | IP SANs checked against peer's resolved IP. **[MTLS-090]** | DNS SANs checked against peer's hostname (if known). **[MTLS-090]** | Connection established. | Handshake fails. |
