...
| **Cert reconfiguration connection handling** | Existing connections unaffected by cert changes. New certs only used for future handshakes. TLS 1.3 has no mid-connection cert re-validation. | Old connections removed from connection map and closed immediately on cert reconfiguration. **[MTLS-085, MTLS-091]** Disrupted requests are retried on a new connection with the new cert. | Standard TLS leaves old connections open indefinitely. Our approach closes old connections immediately since a drain timeout would delay the close without reducing the probability of disrupting an in-progress request. | Positive. Stricter than standard TLS — old connections are closed immediately rather than left open. |
