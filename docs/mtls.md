...
|---|---|---|---|
| **Compromised device cert** | Attacker obtains a device's private key and cert. | Not mitigated. The attacker can authenticate as the compromised device on new connections until the cert expires. `set_cert` only reconfigures the local device's cert — it does not invalidate the old cert for other peers. All commands are validated by the graph regardless. | Cert revocation (CRL/OCSP) to centrally invalidate compromised certs. |
