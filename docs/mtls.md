...
Call `set_cert` again with new file paths per **[MTLS-029]**. The daemon overwrites cert files, replaces the keystore key, and updates the resolver and verifier caches. Existing connections for the team MUST be immediately terminated, including any in-progress sync requests/responses and buffered data. **[MTLS-085]** A cert rotation is a signal that the old cert is no longer trusted — data from connections using the old cert MUST NOT be trusted. New connections are created lazily on the next sync request using the new cert. The server's `ResolvesServerCert` and `ClientCertVerifier` caches are updated immediately, so new inbound connections also use the new cert.
