...
| **Compromised CA** | Attacker compromises a CA in the cert chain and issues fraudulent device certs. | Partially mitigated. Per-team cert chains limit blast radius — a compromised CA can only impact teams that use it. **[MTLS-060]** Cert chains SHOULD NOT be reused across teams to contain the blast radius of a CA compromise. | Cert revocation to invalidate certs issued by the compromised CA. |
