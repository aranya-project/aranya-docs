...
This design intentionally deviates from standard TLS/mTLS conventions in several areas. The following table documents each deviation with its rationale and security impact. Reviewers familiar with the standard mTLS threat model can use this as a delta — rather than auditing the entire TLS security model from scratch, verify that each deviation is security-neutral or security-positive.
...
| **Compromised cert handling** | No special handling beyond standard cert revocation (CRL/OCSP). | Same as cert rotation for now — `set_cert` with a new cert closes old connections immediately. **[MTLS-085, MTLS-091]** Cert revocation (CRL/OCSP) is not yet implemented. | Reconfiguring the device cert is the only current mechanism to stop presenting a compromised cert. The old cert remains valid for new connections from other peers until it expires. | Cert revocation is planned as future work to fully invalidate compromised certs. See [Future Work](#future-work). |
