...
2. Remove all existing connections for this team from the connection map. **[MTLS-085]** No new outbound streams will use the old connection. After a configurable timeout, force-close the connection with `CONNECTION_CLOSE`. **[MTLS-091, MTLS-092]** The timeout allows in-progress streams to complete. The peer receives the close and removes the connection from its own map, so its next request automatically establishes a new connection using the updated cert. If a peer's request fails during the transition, it SHOULD retry — the retry will use a new connection with the new cert. Note: connection handoff during cert reconfiguration will be smoother once automatic retry logic for failed sync requests is implemented (see [Future Work](#future-work)).
```rust
...
| **Cert reconfiguration connection handling** | Existing connections unaffected by cert changes. New certs only used for future handshakes. TLS 1.3 has no mid-connection cert re-validation. | Old connections removed from connection map on cert reconfiguration. **[MTLS-085, MTLS-091]** Force-closed after a configurable timeout to allow in-progress streams to complete. **[MTLS-092]** Peer retries failed requests on a new connection with the new cert. | Standard TLS leaves old connections open indefinitely. Our approach phases out old connections after a short timeout. | Positive. Stricter than standard TLS — old connections are actively phased out rather than left open. Timeout bounds exposure window. |
