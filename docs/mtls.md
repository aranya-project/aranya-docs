...
|---|---|---|---|---|---|---|
| **Outbound — server cert** | **Custom.** Set to team ID (base58), not a hostname. **[MTLS-084]** | Standard mTLS. Trust anchors selected by team ID from SNI. **[MTLS-061]** | Standard TLS. If cert has IP SANs: peer's resolved IP MUST match one. **[MTLS-090]** | Standard TLS. If cert has DNS SANs and peer was configured by hostname: hostname MUST match one. **[MTLS-090]** | Connection established. | Handshake fails. |
